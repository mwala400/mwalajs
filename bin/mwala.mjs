...
// mwala.mjs 
import { displayHelp } from './help.mjs';

if (!command || command === 'help' || command === 'h' || command === '--help' || command === '-h' || command === '--h') {
  displayHelp(pkg);
}
...
case 'db:restore': {
  ...
  console.log(`\ Export folder: ${baseDir}\n`);
...
case 'db:import-all': {
  const folder = args[1]; // exports/2026-03-22
  const format = args[2]?.toLowerCase() || 'all'; // csv, json, sql, au all

  if (!folder) {
    return error('Usage: mwala db:import-all <folder_path> [csv|json|sql|all]');
  }

  await runSafe(
    () => imports.importAllFromFolder(folder, format),
    `Bulk Import (${format}) finished`
  );
  break;
}
